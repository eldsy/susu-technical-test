...
def calcul_balance(user_id: int) -> float:
    ...
    refunds = transactions.transactions_by_type_states(
        db, user_id, TransactionType.REFUND, [TransactionState.COMPLETED, TransactionState.PENDING])
    ...
    return total_amount(completed_deposits) - total_amount(refunds) - total_amount(completed_withdrawals)
